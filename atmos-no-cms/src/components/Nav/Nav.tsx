...
export default function Nav() {
  ...
  // Keep the same luxe ease; just shorten timings below
  const EASE = [0.22, 1, 0.36, 1] as const;

  // Slight container-level stagger so rows get going quickly
  const listGroup: Variants = {
    hidden: {},
    visible: { transition: { staggerChildren: 0.06, when: "beforeChildren" } },
  };

  // Faster & smoother row reveal
  const linkRow: Variants = {
    ...
    visible: (i: number) => ({
      ...
      transition: {
        duration: 0.26,
        ease: EASE,
        delay: 0.05 + i * 0.09, // was 0.08 + i * 0.16
        when: "beforeChildren",
      },
    }),
  };

  // Baseline clip reveal: quicker
  const baselineGroup: Variants = {
    hidden: { clipPath: "inset(100% 0% 0% 0%)" },
    visible: {
      clipPath: "inset(0% 0% 0% 0%)",
      transition: { duration: 0.18, ease: EASE, when: "beforeChildren" },
    },
  };

  // Faster character stagger & tween
  const makeLabelGroup = (charStagger = 0.06, delayChildren = 0.02): Variants => ({
    hidden: {},
  ...
  const labelChar: Variants = {
    ...
    visible: {
      ...
      y: 0,
      scale: 1,
      transition: { duration: 0.20, ease: EASE }, // was 0.25
    },
  };
          ...
          <div className="menu__content container" ref={menuPanelRef} tabIndex={-1}>
            ...
            >
              <div className="menu__links">
                <motion.div variants={linkRow} custom={0} className="menu__linkWrapper" style={{ willChange: "transform, opacity" }}>
                ...
                <motion.div variants={linkRow} custom={1} className="menu__linkWrapper" style={{ willChange: "transform, opacity" }}>
                ...
                <motion.div variants={linkRow} custom={2} className="menu__linkWrapper" style={{ willChange: "transform, opacity" }}>
